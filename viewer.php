...
if( ! isset( $_GET['reserve_ids'] )) jdialog("予約番号が指定されていません", "recordedTable.php");
$reserve_ids = $_GET['reserve_ids'];


try{
	$rrec = new DBRecord( RESERVE_TBL, "id", $reserve_ids[0] );
	...
	foreach( $reserve_ids as $reserve_id){
		...
		if(   $rrec->complete ) echo "<REF HREF=\"".$settings->install_url.$settings->spool."/".urlencode($rrec->path) ."\" />";
